...
class WNmMLP(nn.Module):
    ...
    def __call__(self, X):
        ...
        U = nn.activation.tanh(WN_layer(self.features[0], kernel_init=init)(X))
        V = nn.activation.tanh(WN_layer(self.features[0], kernel_init=init)(X))
        H = nn.activation.tanh(WN_layer(self.features[0], kernel_init=init)(X))
        for fs in self.features[1:-1]:
            Z = WN_layer(fs, kernel_init=init)(H)
